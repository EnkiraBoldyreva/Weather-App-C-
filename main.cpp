   ...
   */
    void plotLineStat(const QVector<QPointF>& xys, const QString& name, QColor color,
        ...
        double stddev = std::sqrt(std::accumulate(
                                      ...
                                      [mean](double sum, double v) { return sum + (v - mean) * (v - mean); } ) / //квадрат отклонения от среднего
                                  values.size());
 ...
 *
 * Поля:
 * - tabs        Вкладки интерфейса (QTabWidget)
 * - cityEdit    Поле ввода города (текущая погода)
 * - cityEditStd Поле ввода города (СКО)
 * - cityEditAvg Поле ввода города (средние)
 * - freqBoxStd  Выбор частоты (СКО)
 * - freqBoxAvg  Выбор частоты (средние)
 * - currLabel   Лейбл текущей погоды
 * - currChart   График текущей погоды
 * - statCharts  Массив графиков (СКО)
 * - avgCharts   Массив графиков (средние)
 * - chartsDialog Окно для показа 4 графиков за сутки
 * - forecastData Данные прогноза для текущей погоды
 */
